import{s as P,r as f,a as U,c as I,j as t,p as O}from"./index-Cpaflyr8.js";import{a as A,g as Z,f as E,C as W,e as B,P as y}from"./utils-CgBJMdzV.js";import{S as H}from"./StatCard-C_gl8h3q.js";const d={nameZh:"展億室內開發有限公司",nameEn:"Door World",brandZh:"門的世界",taxId:"60667469",addr:"新北市五股區成泰路一段130-3號",phone:"02-2292-0366",web:"doorworld.com.tw",tagline:"頂級大門・專業安裝・品質保證"},Q={single:"單門",mother:"子母門",double:"雙開門",fire:"防火單門",room:"房間門",bathroom:"衛浴門",sliding:"橫拉門"},Y={none:"一般（非防火）",f60a:"F60A 防火",f60a_smoke:"F60A 防火遮煙"};function z(e){return e||e===0?"NT$ "+Number(e).toLocaleString():"—"}function F(e){return(e?new Date(e):new Date).toLocaleDateString("zh-TW",{year:"numeric",month:"long",day:"numeric"})}function o(e){return String(e??"").replace(/[<>&"]/g,n=>({"<":"&lt;",">":"&gt;","&":"&amp;",'"':"&quot;"})[n])}async function G(e){if(!e){alert("找不到報價單資料");return}let n="";if(e.product_code)try{const a=await P(`products?full_code=eq.${encodeURIComponent(e.product_code)}&select=thumbnail_url`);n=a&&a[0]&&a[0].thumbnail_url||""}catch{}const i=e.formal_quote_data||{},k=Array.isArray(i.accessories)?i.accessories:[],r=Array.isArray(i.special_requirements)?i.special_requirements:[],x=e.formal_quote_no||e.order_no||e.case_no||"—",c=F(null),S=F(e.created_at),l=Q[e.door_type]||e.door_type||"—",N=Y[i.fire_type]||(e.is_fireproof?"防火":"一般"),_=e.actual_width_cm||(i.width_mm?Math.round(i.width_mm/10):""),v=e.actual_height_cm||(i.height_mm?Math.round(i.height_mm/10):""),C=_&&v?`寬 ${_} × 高 ${v} cm`:"—",h=e.quantity||1,u=k.map(a=>{const b=a.useUpgrade&&a.upgrade?a.upgrade:a.standard,w=a.useUpgrade&&a.upgrade?"升級":"標配";return{label:a.label||a.key,item:b||"—",level:w}}).filter(a=>a.item&&a.item!=="—"),j=u.length===0?"":`
    ...
    <table class="bd-table">
      ...
      ${u.map(a=>`<tr>
        <td class="bdl">${o(a.label)}</td>
        <td class="bdv" style="text-align:left;font-weight:500">${o(a.item)}</td>
        <td class="bdv" style="text-align:center;font-size:9px;color:${a.level==="升級"?"#c9a227":"#888"}">${a.level}</td>
      </tr>`).join("")}
    </table></div>`,$=[["開門方向",i.door_direction],["交貨方式",i.delivery_type],["安裝方式",i.install_method],["有無電梯",i.has_elevator===!0?"有":i.has_elevator===!1?"無":null],["畫框",i.art_frame&&i.art_frame!=="無"?i.art_frame:null]].filter(([,a])=>a),s=$.length===0?"":`
    <div class="sec"><div class="stitle"><span class="stitle-txt">安裝規格</span></div>
    <table class="spec-grid">
      ${$.reduce((a,b,w)=>(w%2===0?a.push([b]):a[a.length-1].push(b),a),[]).map(a=>`<tr>${a.map(([b,w])=>`<td class="tdl">${o(b)}</td><td class="tdv">${o(w)}</td>`).join("")}${a.length===1?'<td class="tdl" style="border:none;background:none"></td><td class="tdv" style="border:none;background:none"></td>':""}</tr>`).join("")}
    </table></div>`,p=r.length===0?"":`
    <div class="sec"><div class="stitle"><span class="stitle-txt">特殊需求</span></div>
    <div class="special-box">${r.map(a=>`<span class="tag">${o(a)}</span>`).join("")}</div></div>`,g=Number(e.total_with_tax||0),m=g?Math.round(g/1.05):Number(e.official_price||0),T=g?g-m:Math.round(m*.05),R=g||m+T,D=h>0?Math.round(m/h):m,L=n?`<img src="${o(n)}" alt="" style="width:120px;height:120px;object-fit:cover;border:1px solid #d4af37;display:block">`:'<div style="width:120px;height:120px;border:1px dashed #d4af37;display:flex;align-items:center;justify-content:center;font-size:9px;color:#c9a227;letter-spacing:1px">暫無圖片</div>',M=`<!DOCTYPE html><html><head>
<meta charset="utf-8"><title>報價單 ${o(x)}</title>
...
<div class="page">
  <div class="hdr">
    ...
    <div class="hdr-r">
      ...
      <div class="qno">${o(x)}</div>
      <span class="badge">正式報價</span>
      <div class="doc-date">${S}</div>
    </div>
  </div>
  <div class="infobar">
    <div class="ic"><div class="icl">客戶姓名</div><div class="icv">${o(e.customer_name||"—")}</div></div>
    <div class="ic"><div class="icl">聯絡電話</div><div class="icv">${o(e.customer_phone||"—")}</div></div>
    ${e.sales_person?`<div class="ic"><div class="icl">業務窗口</div><div class="icv">${o(e.sales_person)}</div></div>`:""}
  </div>
  ${e.case_address?`<div class="addr"><span class="addr-lbl">施工地址</span>${o(e.case_address)}</div>`:""}

  <div class="sec"><div class="stitle"><span class="stitle-txt">產品資訊</span></div>
    <div class="prod-row">
      <div class="prod-info"><table>
        <tr><td class="tdl">產品編號</td><td class="tdv" style="font-family:monospace;font-size:12px;font-weight:700">${o(e.product_code||"—")}</td></tr>
        <tr><td class="tdl">門型</td><td class="tdv">${o(l)}</td></tr>
        <tr><td class="tdl">防火規格</td><td class="tdv">${o(N)}</td></tr>
        <tr><td class="tdl">尺寸規格</td><td class="tdv">${o(C)}</td></tr>
