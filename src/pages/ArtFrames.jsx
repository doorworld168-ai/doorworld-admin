...
export default function ArtFrames() {
  ...
  async function load() {
    setLoading(true);
    try { setRows(await sbFetch('art_frames?select=*&order=sort_order.asc,code.asc') || []); }
  ...
  async function uploadImage(file) {
    ...
    setUploading(true);
    try {
      const url = await uploadFile('art-frames', fileName, file);
      setImageUrl(url);
      toast('圖片已上傳', 'success');
  ...
  async function save(e) {
    ...
    try {
      if (modal.data?.id) await sbFetch(`art_frames?id=eq.${modal.data.id}`, { method: 'PATCH', body: JSON.stringify(body) });
      else await sbFetch('art_frames', { method: 'POST', body: JSON.stringify(body) });
      toast('已儲存', 'success');
  ...
  function del(p) {
    confirm('確認刪除？', `${p.code} ${p.name_zh || ''} 將永久刪除。`, async () => {
      await sbFetch(`art_frames?id=eq.${p.id}`, { method: 'DELETE' });
      toast('已刪除', 'success');
      load();
    });
  }
  ...
  async function toggleActive(p) {
    await sbFetch(`art_frames?id=eq.${p.id}`, { method: 'PATCH', body: JSON.stringify({ is_active: !p.is_active }) });
    load();
  }
  ...
  const filtered = rows.filter(r => {
    ...
    return (r.code || '').toLowerCase().includes(s) || (r.name_en || '').toLowerCase().includes(s) || (r.name_zh || '').toLowerCase().includes(s);
  });
    ...
    <div>
      <div className="page-header">
        <div className="page-title-wrap">
          <div className="page-title">藝術框管理</div>
          <div className="page-subtitle">管理門扇藝術框 — 編號、樣品圖、名稱</div>
        </div>
        <button className="btn btn-primary" onClick={() => openModal(null)}>+ 新增藝術框</button>
      </div>

      <div style={{ display: 'flex', gap: 10, marginBottom: 14, alignItems: 'center', flexWrap: 'wrap' }}>
        <input placeholder="搜尋編號 / 名稱..." value={search} onChange={e => setSearch(e.target.value)} style={{ ...inputStyle, width: 280 }} />
      ...
      {loading ? <div className="loading"><div className="spinner" /><br />載入中...</div> :
        filtered.length === 0 ? <div className="empty"><div className="icon">🖼</div>{search ? '查無符合' : '尚無藝術框，請新增'}</div> :
      ...
      <Modal open={modal.open} onClose={() => setModal({ open: false, data: null })} title={d.id ? '編輯藝術框' : '新增藝術框'} maxWidth={560}
        footer={<><button className="btn btn-ghost" onClick={() => setModal({ open: false, data: null })}>取消</button><button className="btn btn-primary" type="submit" form="af-form">儲存</button></>}>
        <form id="af-form" onSubmit={save} className="form-grid">
          <div className="form-group"><label>編號 *</label><input name="code" defaultValue={d.code || ''} required placeholder="例：AF-001" /></div>
          <div className="form-group"><label>分類</label><input name="category" defaultValue={d.category || ''} placeholder="例：方框、圓弧" /></div>
