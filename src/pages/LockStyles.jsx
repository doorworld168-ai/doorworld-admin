...
export default function LockStyles() {
  ...
  async function load() {
    setLoading(true);
    try { setRows(await sbFetch('lock_styles?select=*&order=sort_order.asc,code.asc') || []); }
  ...
  async function uploadImage(file) {
    ...
    setUploading(true);
    try {
      const url = await uploadFile('lock-styles', fileName, file);
      setImageUrl(url);
      toast('圖片已上傳', 'success');
  ...
  async function save(e) {
    ...
    try {
      if (modal.data?.id) await sbFetch(`lock_styles?id=eq.${modal.data.id}`, { method: 'PATCH', body: JSON.stringify(body) });
      else await sbFetch('lock_styles', { method: 'POST', body: JSON.stringify(body) });
      toast('已儲存', 'success');
  ...
  function del(p) {
    confirm('確認刪除？', `${p.code} ${p.name_zh || ''} 將永久刪除。`, async () => {
      await sbFetch(`lock_styles?id=eq.${p.id}`, { method: 'DELETE' });
      toast('已刪除', 'success');
      load();
    });
  }
  ...
  async function toggleActive(p) {
    await sbFetch(`lock_styles?id=eq.${p.id}`, { method: 'PATCH', body: JSON.stringify({ is_active: !p.is_active }) });
    load();
  }
  ...
  const filtered = rows.filter(r => {
    ...
    return (r.code || '').toLowerCase().includes(s) || (r.name_en || '').toLowerCase().includes(s) || (r.name_zh || '').toLowerCase().includes(s) || (r.brand || '').toLowerCase().includes(s);
  });
    ...
    <div>
      <div className="page-header">
        <div className="page-title-wrap">
          <div className="page-title">門鎖樣式管理</div>
          <div className="page-subtitle">管理門鎖樣式 — 編號、品牌、樣品圖</div>
        </div>
        <button className="btn btn-primary" onClick={() => openModal(null)}>+ 新增鎖樣式</button>
      </div>

      <div style={{ display: 'flex', gap: 10, marginBottom: 14, alignItems: 'center', flexWrap: 'wrap' }}>
        <input placeholder="搜尋編號 / 名稱 / 品牌..." value={search} onChange={e => setSearch(e.target.value)} style={{ ...inputStyle, width: 280 }} />
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer', color: 'var(--text-muted)' }}>
          <input type="checkbox" checked={showInactive} onChange={e => setShowInactive(e.target.checked)} style={{ accentColor: 'var(--gold)' }} />顯示已停用
        </label>
      ...
      {loading ? <div className="loading"><div className="spinner" /><br />載入中...</div> :
        filtered.length === 0 ? <div className="empty"><div className="icon">🔐</div>{search ? '查無符合' : '尚無門鎖樣式，請新增'}</div> :
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
          {filtered.map(p => (
            <div key={p.id} style={{ background: 'var(--surface-low)', borderRadius: 'var(--radius)', overflow: 'hidden', border: `1px solid ${p.is_active ? 'var(--border)' : 'rgba(239,68,68,.3)'}`, opacity: p.is_active ? 1 : 0.6 }}>
              <div style={{ width: '100%', height: 180, background: 'var(--surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ padding: '12px 14px' }}>
                ...
                {p.brand && <div style={{ fontSize: 11, color: 'var(--gold)', marginBottom: 2 }}>{p.brand}</div>}
      ...
      <Modal open={modal.open} onClose={() => setModal({ open: false, data: null })} title={d.id ? '編輯鎖樣式' : '新增鎖樣式'} maxWidth={560}
        footer={<><button className="btn btn-ghost" onClick={() => setModal({ open: false, data: null })}>取消</button><button className="btn btn-primary" type="submit" form="ls-form">儲存</button></>}>
        <form id="ls-form" onSubmit={save} className="form-grid">
          <div className="form-group"><label>編號 *</label><input name="code" defaultValue={d.code || ''} required placeholder="例：LS-001" /></div>
          <div className="form-group"><label>品牌</label><input name="brand" defaultValue={d.brand || ''} placeholder="例：MIWA、加安" /></div>
          <div className="form-group"><label>中文名稱</label><input name="name_zh" defaultValue={d.name_zh || ''} placeholder="例：機械水平把手鎖" /></div>
          <div className="form-group"><label>英文名稱</label><input name="name_en" defaultValue={d.name_en || ''} /></div>
          <div className="form-group"><label>分類</label><input name="category" defaultValue={d.category || ''} placeholder="例：機械鎖、電子鎖" /></div>
